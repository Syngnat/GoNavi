...
flowchart LR
    ...
    Worker --> DMIT
    Worker -. "DMIT 请求失败" .-> BERO
    Worker -. "DMIT 与 Bero 请求均失败" .-> GH["GitHub Releases"]
```
...
- GitHub Releases 是最后灾备源。GatewaySentry（旧地址 `157.254.234.28`）、腾讯节点和 Cloudflare R2 不参与上传或下载调度。
...
5. 只有 DMIT 与 Bero 均完成验证后，发布才成功；两个节点都必须切换到同一 generation。Dispatcher 不写入或读取远程路由控制状态。
6. 如果任一节点上传、校验、公开 Range 或 health 验证失败，发布失败；不能只发布 DMIT 后把 Bero 留在旧 generation，也不能回退到 GatewaySentry。
...
- DMIT 域名和 Bero Cloudflare 域名的 Range、health HTTP 请求最多 60 秒，8 路吞吐观测每个节点每路最多 120 秒。
- 所有受控命令超时后先发 `TERM`，15 秒后强杀。吞吐观测失败仅记录 `limited` 告警，但 Range 与 health 未通过时发布不得成功。
- 任一节点失败都应使本次发布失败，防止两个源的 generation 分裂。客户端运行时仍按 DMIT、Bero、GitHub 的顺序回退。

## Worker 和客户端行为

Worker 只维护两个 canonical 节点，固定顺序为 `dmit`、`bero`，并在每次请求中直接生成静态候选列表。它不读取、写入或定时刷新 Cloudflare KV，也不根据运行时健康状态改变候选顺序。发布脚本仍会在切换 generation 前验证每个节点的 HTTPS、`/healthz` 和 immutable `206`，因此健康检查失败会阻止错误版本上线。

Bero URL 固定为 `https://origin-download.syngnat.top:8443`，请求路径与 DMIT 完全相同，流量经 Cloudflare 代理到 `94.103.173.47:8443`。旧 KV 中的 control、`CDN_NETCUP_*` secrets/variables 即使暂时存在也不会被新 Worker、workflow、action 或发布脚本读取。
...
旧客户端的 `302` 只返回默认 DMIT URL；它本身没有候选列表，因此不会在 Worker 侧根据 KV、健康状态或 generation 改写到 Bero。
...
- 部署 Worker：`CLOUDFLARE_WORKERS_API_TOKEN` 和 `CLOUDFLARE_ACCOUNT_ID`。

还需要以下 repository variable：

- `CDN_BERO_BASE_URL`：固定为 `https://origin-download.syngnat.top:8443`，必须与 DNS、Origin CA 证书和 Worker 的静态 Bero 候选一致；不能是 IP、DNS-only 主机、DMIT 域名或 GatewaySentry 域名。
...
4. 手动部署 `Deploy Download Dispatcher`，确认 `npm run check` 通过后再部署 Worker。该版本直接返回静态 `dmit`、`bero`、`github` 候选，不依赖旧 KV 中的 generation 或节点状态。
5. 运行一次 dev build 或 stable publish，确认两个节点都得到同一 generation，且 `candidates` 顺序为 `dmit`、`bero`、`github`。
6. 模拟 DMIT 下载失败：Dispatcher JSON 仍应按 `dmit`、`bero`、`github` 返回完整候选链；客户端应在 DMIT Range/校验失败后转向 Bero，再在 Bero/Cloudflare 失败后转向 GitHub。旧式 `302` 仅指向默认 DMIT，不能单独表达后续候选。
7. 验证客户端对 JSON 中三个候选都执行 Range/校验失败回退；确认 Worker 从不代理大文件，Bero fallback 的实际 TCP/HTTPS 入口是 Cloudflare `:8443` 而不是源站 IP。

不要在本次变更中删除 DMIT、修改 DMIT 的 Caddy 监听器或删除旧 GatewaySentry 服务器；这些是独立的运维变更。DNS、旧 secrets、遗留 KV 数据和客户端缓存中若还存有旧地址，应在 Bero 发布和公网验收稳定后单独下线，不能让新 CI 误上传过去。
