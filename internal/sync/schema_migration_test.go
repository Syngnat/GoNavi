...
func TestBuildPGLikeToPGLikePlan_AutoCreateWhenTargetMissing(t *testing.T) {
	t.Parallel()
	...
	sourceDB := &fakeMigrationDB{
		columns: map[string][]connection.ColumnDefinition{
			"public.orders": {
				{Name: "id", Type: "bigint", Nullable: "NO", Key: "PRI", Extra: "auto_increment"},
				{Name: "created_at", Type: "timestamp without time zone", Nullable: "NO", Default: stringPtr("CURRENT_TIMESTAMP")},
	...
	plan, sourceCols, targetCols, err := buildPGLikeToPGLikePlan(cfg, "orders", sourceDB, targetDB)
	if err != nil {
	...
	if !strings.Contains(plan.CreateTableSQL, `"id" bigint GENERATED BY DEFAULT AS IDENTITY NOT NULL`) {
		t.Fatalf("expected identity mapping, got: %s", plan.CreateTableSQL)
	}
	if !strings.Contains(plan.CreateTableSQL, `"created_at" timestamp without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL`) {
		t.Fatalf("expected default preservation, got: %s", plan.CreateTableSQL)
	}
	if len(plan.PostDataSQL) != 1 || !strings.Contains(plan.PostDataSQL[0], `CREATE INDEX "idx_orders_created_at"`) {
		t.Fatalf("unexpected post-data SQL: %v", plan.PostDataSQL)
	}
}
...
func TestBuildSchemaMigrationPlan_MySQLToPostgresAutoCreateWhenTargetMissing(t *testing.T) {
	t.Parallel()
	...
	if !strings.Contains(plan.CreateTableSQL, `GENERATED BY DEFAULT AS IDENTITY`) {
		t.Fatalf("expected identity mapping in create table SQL: %s", plan.CreateTableSQL)
	}
	if len(plan.PostDataSQL) != 1 || !strings.Contains(plan.PostDataSQL[0], `CREATE INDEX "idx_orders_user"`) {
		t.Fatalf("unexpected post-data SQL: %v", plan.PostDataSQL)
	}
}
