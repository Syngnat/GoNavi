...
func TestAcquireReleaseAndSecondInstance(t *testing.T) {
	...
	if err := first.Handle.Listen(name, func() error {
		received <- struct{}{}
		return nil
	}); err != nil {
...
func TestSecondInstanceWaitsForDelayedListener(t *testing.T) {
	...
	go func() {
		...
		done <- primary.Handle.Listen(name, func() error {
			received <- struct{}{}
			return nil
		})
	}()
...
func TestEndpointLifecycle(t *testing.T) {
	...
	if err := h.Handle.Listen(name, func() error { return nil }); err != nil {
		t.Fatalf("listen: %v", err)
	}
