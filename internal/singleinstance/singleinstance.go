...
func Acquire(name string) AcquireResult {
	name = normalizeName(name)
	lock, err := platformAcquireLock(name)
	if err != nil {
		if isLockHeldError(err) {
			notifyErr := notifyPrimaryWithRetry(name)
...
func (h *Handle) handleConn(conn net.Conn, handler ActivationHandler) {
	...
	if string(line) != activationSignal {
		packageLogger.Warnf("single-instance received invalid activation signal")
		return
	}
...
func notifyPrimaryWithRetry(name string) error {
	deadline := time.Now().Add(notifyRetryWindow)
	var lastErr error
	for {
		lastErr = notifyPrimary(name)
		if lastErr == nil {
			return nil
		}
...
func notifyPrimary(name string) error {
	...
	if _, err := conn.Write([]byte(activationSignal + "\n")); err != nil {
